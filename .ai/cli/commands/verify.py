...
def _load_safety_policy() -> Dict[str, Any]:
    """Load .ai/policies/safety.yaml; return {} on any failure (defensive)."""
    try:
        ssot = SSOTLoader()
        policy_path = Path(ssot.get_path("policies")) / "safety.yaml"
    except Exception:
        # Fallback: walk up from this file to find the kernel root.
        policy_path = Path(__file__).resolve().parents[2] / "policies" / "safety.yaml"
    try:
...
class VerificationGates:
    ...
    def __init__(self, target_dir: Path, config: Dict[str, Any]):
        ...
        self.config = config
        self.policy = _load_safety_policy()
        self.exclude_paths: List[str] = (
            self.policy.get("gates", {})
            .get("secrets", {})
            .get("exclude_paths", [])
        )
        self.results = {
    ...
    def _is_excluded(self, file_path: Path) -> bool:
        """True if file_path matches any glob in safety.yaml's exclude_paths."""
        ...
        try:
            ...
            return False
        # fnmatch handles ** as a non-greedy wildcard at the path level when
        # combined with PurePath.match; use both for safety.
        for pattern in self.exclude_paths:
            ...
            try:
                ...
                pass
            # ** at end means "any depth under prefix"
            if pattern.endswith("/**"):
                prefix = pattern[:-3]
    ...
    def check_secrets(self) -> bool:
        ...
        for file_path in self.target_dir.rglob("*"):
            ...
            # Skip paths excluded by safety.yaml (E2E F2 fix)
...
def run_verify(scope: str, strict: bool):
    ...
    # F1: append verify.completed event to global audit chain (D9)
    try:
        ...
        chain.append("verify.completed", {
            ...
            "passed": overall_pass,
            "mode": mode_str,
            "blocks_count": len(report["blocks"]),
        })
    except Exception as e:
