---
title: "Trinity Goal Tree + Loop Specification"
subtitle: "Goal-directed execution with sub-goals, checkpoints, resume"
version: 1.0.0-draft
status: draft
last-updated: 2026-04-28
phase: 5
critical-fix: "Linear loop ไม่พอ — ต้องรองรับ goal tree + checkpoint"
---

# Trinity Goal Tree + Loop Specification v1.0

> **Loop คือหัวใจของการทำจนจบ — ไม่ใช่แค่ run-once script**
>
> Loop ต้องรู้: เป้าหมายคืออะไร · sub-goal มีอะไร · ทำไปถึงไหน · resume ตรงไหน · stop เมื่อไร
...
- **Critical fix from feedback:** เดิม blueprint มี loop แค่ linear `lll → vvv → nnn → gogogo → rrr` — ไม่พอสำหรับงานใหญ่

---

## 1. Why Goal Tree

### Linear loop (เดิม) — ไม่พอ

```text
user: "ทำ SEO ทั้งเว็บ"
→ lll → vvv → nnn → gogogo → rrr → DONE? ❌
```

**ปัญหา:**
- ทำ "ทั้งเว็บ" = 50+ pages
- ทำเสร็จหน้าเดียว = ไม่ done
- ไม่มี way to track sub-goals
- รีสตาร์ทกลางทาง = เริ่มใหม่หมด

### Goal Tree (ใหม่)

```text
g_001 [epic] "ทำ SEO ทั้งเว็บ"
...
**ผลลัพธ์:**
- Decompose ครั้งเดียว → sub-goals ทำต่อกันได้
- Resume ตรงสมบูรณ์ — เห็นว่า g_007 ทำเสร็จ, g_008 pending
- Aggregate verdict — root goal PASS = ทุก leaf PASS
...
- ทุก state change → also append to `events.ndjson`
...
```bash
trinity loop --goal "ทำ SEO ทั้งเว็บ" [--config path] [--max-iter N] [--budget tokens=200000]
```
...
- Trinity kernel locks prevent race
