---
title: "<upstream-project> — Trinity OS Migration Audit"
...
# <upstream-project> — Trinity OS Migration Audit

> **Audit ของ <upstream-project> vs Trinity OS v2 spec — gap analysis + migration plan**
>
> <upstream-project> = production project ที่ run มา 6+ เดือน · ห้ามทำลาย · ต้อง migrate แบบ additive
...
```text
❌ memory-cli — ไม่มี → 240 retros ค้นเฉพาะ grep
❌ retro-cli — ไม่มี → retros free-form, no schema
❌ verify-cli + verifier-rules.yaml — มี vvv คำสั่ง แต่ไม่มี deterministic rules
❌ Tool Contract compliance — browser-cli ใช้ pattern คล้าย แต่ไม่ official
❌ Goal Tree + Loop State — workflow ยัง manual sequence
❌ Graph YAML + transition authority — มี graph แต่ implicit
❌ Trinity Shim adapter — มี skills แต่ไม่ตาม spec
⚠️  MCP servers (Playwright/Morphllm/Sequential) — ต้องลบ (decision #5)
```
...
> ✅ **<upstream-project>/ai-docs ดีกว่า Bootstrap Pack baseline** — keep as-is, just **index into memory-cli**
...
- **Size:** 11KB
- **Quality:** ⭐ Inline short codes ที่บรรทัด 46-52 (good!)
- **Pattern:** vendor-aware (Codex/Gemini/Claude task assignment matrix)

#### vs Trinity OS v2 Spec

✅ **<upstream-project> AGENTS.md is reference-quality** — ใช้เป็น template สำหรับ Bootstrap Pack
...
❌ **MISSING** — <upstream-project> ไม่มี GEMINI.md ที่ root
...
**Deliverable:** browser-cli is reference-implementation per spec
...
**Deliverable:** <upstream-project> 100% CLI-first (no MCP dep)
...
1. browser-cli capability — does it cover ทุก Playwright MCP 13 tools?
2. memory-cli ใช้ Node หรือ Python (เพื่อ consistency กับ kernel Python)?
3. retros 240 — บางตัว draft คุณภาพต่ำ — ทำไง?
4. <upstream-project> production timing — รับ migration window ตอนไหน?
5. Multi-project — <upstream-project> + new projects share tools dir?
6. Memory-cli location — `~/.memory/` ของแต่ละ project หรือกลาง?
7. MCP removal — ทำพร้อมกัน หรือทีละตัว?
...
>
> **Don't touch production workflow until P4 complete (MCP cleanup)**
>
> **Phase 2 (memory-cli + index 240 retros) = highest ROI** — unlocks semantic recall ทันที
>
> **Don't rush — 17-week timeline realistic, not 4 weeks**
