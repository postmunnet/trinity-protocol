---
title: "Trinity Shim Specification v1.0 (English)"
...
phase: 8
note: "Translation of ../07_SHIM_SPEC.md"
purpose: "Bridge between vendor AI harness and Trinity kernel without rebuilding harness"
---

# Trinity Shim Specification v1.0 (English)

> **Trinity Shim = a thin "shell" on top of vendor harnesses (Claude Code/Codex/Cursor/Gemini).**
> 
> Makes vendor harnesses behave like the Trinity workflow ritual — without rewriting the harness.

---

## 0. Status

- **Phase:** 8 (last layer before extension platform)
- **Depends on:** Phase 1-7 (all foundations)
- **Action namespace:** N/A (this is an integration layer, not a standard tool)
...
## 1. Why a Shim (not a new harness)

### Each vendor harness has its own extension model
...
```
Use each vendor's extension as an "adapter"
Forward data → Trinity kernel CLI
Get response back → vendor harness displays
```

We do NOT:
- ❌ Replace vendor harness
- ❌ Build our own conversation loop
- ❌ Build our own LLM call layer
...
1. **Universal Shell** (`trinity-shell`) — single CLI that vendor adapters call
...
# Standard
trinity-shell lll
trinity-shell vvv "Do SEO across the whole site"
trinity-shell nnn
...
3. Render the data as a status report
4. Show:
   - Git status
...
End with: "Ready — type vvv to begin verification"
```
...
Codex CLI doesn't have skills/hooks like Claude Code — use **`AGENTS.md`** + tool config.
...
Before any task, run a mental check:
...
Gemini supports `--system` flag — bake in the Trinity ritual:
...
`.ai/gemini-system.md`:
```markdown
You are an AI assistant working under the Trinity workflow.
...
When the vendor harness calls a Trinity tool (via shell):
```json
{
  "ts": "...",
  "event": "tool_call",
