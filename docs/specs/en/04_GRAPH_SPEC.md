---
title: "Trinity Graph Specification v1.0 (English)"
...
phase: 6
note: "Translation of ../04_GRAPH_SPEC.md"
critical-fix: "Graph must declare decided_by — AI is not allowed to decide transitions"
---

# Trinity Graph Specification v1.0 (English)

> **Graph = workflow skeleton (not the brain).**
>
> AI may propose a transition. AI may NOT decide a transition.
...
- **Critical fix from feedback:** Graph blueprint did not specify *who* changes state — must declare `decided_by`

---

## 1. Why Two-Layer Graph

### Problem with single-layer graph
- Mixes runtime (RUNNING/IDLE) with workflow (THINK/DO/PROMOTED) → confusing
- Workflow varies per project — can't use a single graph
...
- Both run together — Layer 1 manages process, Layer 2 tracks work
...
- **Stable** — rarely changes (versioned)
- **Minimal** — only 6 states
- **No domain logic** — knows nothing about THINK/DO/PROMOTED
...
```text
AI may PROPOSE a transition (via tool call requesting trigger event).
AI may NOT DECIDE a transition.
...
- `true` — even if `decided_by` ≠ human, the human must confirm
- `false` — proceeds automatically

Use this for "double check" — e.g. verifier says PASS but it's a sensitive op → human must still approve.
...
trinity graph transition <event>         # manually fire trigger
...
Decision: start simple (custom Python in Trinity kernel)
...
Should match `loop_state.json` — if not, audit log is corrupted.
...
|-----------------|-----------|
| AI decides transition | `decided_by: verifier/policy/human/kernel` |
| `decided_by` not specified | Required field |
| Transition without trigger event | Every transition has trigger |
| Multiple `to` for same `from+trigger` | Unique transition |
| State has name but isn't used | Validate reachability |
| Embed graph in code | Always YAML file |
| Every project uses same graph | Allow per-project graphs |
