---
title: "Trinity Bootstrap Pack"
subtitle: "Minimal pack to scaffold Trinity into a new project"
version: 1.0.0-draft
status: draft
last-updated: 2026-04-28
phase: 0.5
solves: "Copy ai-docs/Trinity ไป project ใหม่ → AI ไม่รู้ short codes"
---

# Trinity Bootstrap Pack v1.0

> **แก้ปัญหา:** *"copy 2 folder ไป project ใหม่ AI ไม่รู้ short codes มาตรฐานโดยทันที vvv"*

---

## 0. Goal

หลังรัน install.sh ใน project ใหม่ — **AI agent (ทุกตัว) ต้องรู้ทันที** ว่า:
- Short codes คืออะไร (`lll/vvv/nnn/gogogo/rrr`)
- Workflow ที่ต้องตาม
- Tools ที่มี
- Policy ที่ห้ามผิด
- Memory layer (ai-docs) อยู่ที่ไหน

**ไม่ต้องบอก AI ก่อน** — เปิด session ปุ๊ป ใช้งานได้ปั๊ป
...
### Reality ปัจจุบัน
- AI tools (Claude Code/Codex/Gemini/Cursor) อ่านเฉพาะ **entrypoint files** ที่ root
- Short codes อยู่ลึกใน `ai-docs/core/SHORT_CODES.md`
- ถ้าไม่มี entrypoint pointing ไปยัง short codes → AI ไม่รู้

### Bootstrap Pack solves
- Inline short codes ใน entrypoint (CLAUDE.md/AGENTS.md/GEMINI.md)
- Setup minimal `.ai/` + `ai-docs/` ที่ AI เห็นได้ทันที
...
1. **vvv before nnn** — Verify ก่อนวางแผน (100% enforcement)
...
```bash
พิมพ์: lll        # ดูสถานะโปรเจกต์ + session start
```

---

## 📋 Short Codes (Inline — AI ต้องรู้ทันที)
...
- `CLAUDE.md` — fill {{}} placeholders ที่ install.sh ไม่ได้ render
...
1. install.sh = bash หรือ Node script?
2. Templates ใช้ Handlebars/Mustache หรือ sed?
3. ควรมี TUI สำหรับ install ไหม?
4. Verify script ลึกแค่ไหน — แค่ file exist หรือ schema validate?
5. Update path — version migration ทำยังไง?
6. Multi-language project — install.sh detect tech stack อัตโนมัติ?
7. Bootstrap pack version vs Trinity version — sync?
8. Pack distribution — git submodule, npm, curl, brew?

---

## 11. ปัญหาตั้งต้น vs Solution

| Pain | Solution in Pack |
|------|------------------|
| "Copy ai-docs ไป project ใหม่ AI ไม่รู้ short codes" | ✅ CLAUDE.md/AGENTS.md/GEMINI.md inline short codes |
| ทุก project ต้อง manual setup | ✅ install.sh prompts + renders |
| ลืม sync rules ระหว่าง projects | ✅ Pack เป็น single source — copy fresh |
| Codex/Gemini ไม่อ่าน CLAUDE.md | ✅ AGENTS.md / GEMINI.md แยก |
| ไม่รู้ว่า install สมบูรณ์ไหม | ✅ verify-install.sh |
| Custom path ของแต่ละ project | ✅ {{APP_DIR}} / {{DEV_DIR}} / {{PROD_DIR}} placeholders |
