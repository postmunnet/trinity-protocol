---
title: "Trinity Verifier Specification"
subtitle: "Deterministic Judge with file-based rules"
version: 1.0.0-draft
status: draft
last-updated: 2026-04-28
phase: 4
critical-fix: "Judge ต้องมีกฎจริง ไม่ใช่ AI เดา"
---

# Trinity Verifier Specification v1.0

> **Verifier (verify-cli) = Judge ของ Trinity — ตัดสินผลงาน deterministic ด้วยกฎที่อยู่ในไฟล์ ไม่ใช่ใน LLM**
...
- **Critical fix from feedback:** Verifier blueprint บอกแค่ "Judge" — ต้องมีกฎจริง

---

## 1. Why Verifier (= Judge)

### Problem
ถ้าให้ AI ตัดสินเอง:
- "ผ่านไหม?" — AI จะบอก "ผ่าน" เพราะอยากเอาใจ
- ไม่มี evidence — AI จะแต่งเรื่อง
- กฎไม่อยู่ในไฟล์ — drift แน่ ๆ

### Solution
**กฎเป็นไฟล์, ตรวจ deterministic ก่อน, escalate to LLM/human เฉพาะที่จำเป็น**
...
> **R17 — `defaults` block clarification.**
>
> The top-level `defaults:` mapping is an *evaluation pre-fill*, not a
> rule. The kernel verifier engine does **not** read or evaluate
> `defaults` directly. Instead, `cli/commands/gogogo.py` looks up the
> active `rule_set`, then **merges its `defaults` map into the
> step-evidence dict** before calling the verifier (see
> `cli/commands/gogogo.py::_verify_step`). This lets a permissive
> rule set like `step_complete` declare:
>
> ```yaml
> step_complete:
>   pass_when: [step_done]
...
> ```
>
> so any step that doesn't supply richer evidence still PASSes —
> while a step that explicitly sets `step_done: false` (or supplies
> negative evidence in `retry_when` / `dead_when`) still flips the
> verdict. Only the **rule_set's own** `defaults` are merged; the
> top-level file `defaults:` (e.g. `retry_budget`, `timeout_ms`) is
> consumed by the loop runner, not the verifier.
>
> Convention: prefer adding a `defaults` block to a rule set over
> writing a separate force-PASS code path. Keeping the override in
> YAML keeps the rule auditable and editable without code changes.
...
ทุก check ต้องอ้าง evidence type จาก library กลาง:
...
> **R22 — implementation note (2026-05).** This spec was originally
> drafted with `verify-cli` as a **subprocess sibling** alongside
> `memory-cli`, `retro-cli`, etc. In practice it landed as a **kernel
> module with a CLI surface**, not a subprocess sibling: the engine
> lives at `cli/core/verifier.py` and is exposed via the `ai verify`
> subcommand. Per-step verifier calls in `gogogo` bypass the JSON
> envelope and call `evaluate_step()` directly — by design, because
> the hot path runs ~10–50× per session and the rules engine is pure
> dict-lookup against a YAML file (zero deps, deterministic).
>
> The decision was formalized as the cost rule (D14 candidate, see
> `01_CONTEXT_AND_DECISIONS.md`):
>
> > **Kernel-internal hot-path bypasses subprocess boundary even when
> > capability is sibling-shaped.**
>
> The interface contract below (4.1–4.5) still describes the canonical
> CLI surface — both the `ai verify` subcommand and any future
> standalone `verify-cli` sibling MUST honor this shape so a tool
> consumer can swap implementations transparently.
...
- Layer 1 + Layer 2 ทั้งคู่ unsure
...
- Override policy explicit forbid
...
→ Append to `events.ndjson` (hash-chain ตาม blueprint)
...
|-----------------|-----------|
| AI ตัดสิน PASS เอง | verifier ตรวจ rules + evidence |
| Rule อยู่ใน prompt/code | Rule อยู่ใน `verifier-rules.yaml` |
| Verdict = boolean | Verdict = 4 types (PASS/RETRY/NEEDS_HUMAN/DEAD) |
| Evidence = "I think it works" | Evidence = artifact file with sha256 |
| LLM judge ทุกครั้ง | LLM judge เฉพาะ Layer 1+2 unsure |
...
1. Rule YAML — schema validation (JSON Schema) ใช้ตอน load?
2. Evidence library — รวมใน verifier-rules.yaml หรือแยก `evidence-types.yaml`?
3. LLM judge — ใช้ vendor's LLM (Claude API) หรือ local?
4. Human escalation UX — CLI prompt, file-based, หรือ Slack/email?
5. Retry budget — global per session หรือ per rule set?
6. Verdict cache — รัน verify ซ้ำได้เลย หรือต้อง invalidate?
7. Evidence freshness — kept ใช้ได้กี่วินาที?
8. Multi-rule verification — รวม pass = AND หรือ OR?
9. Rule inheritance — `code_change` extends `default`?
10. Custom evidence types — โครงการเพิ่มเองได้ไหม?
...
### Authority
```text
Verifier may decide PASS/RETRY/DEAD on own.
